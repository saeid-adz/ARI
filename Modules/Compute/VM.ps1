            ...
            foreach ($1 in $vm) 
                {
                    ...
                    Write-Output $VMNICS
                    If ($VMNICS -eq '0') {Write-Color $VMNICS -Color Green}           
                    foreach ($2 in $VMNICS) {
                        ...
                        $PIP = if($vmnic.properties.ipConfigurations.properties.publicIPAddress.id){$vmnic.properties.ipConfigurations.properties.publicIPAddress.id.split('/')[8]}else{'None'}
                        ...
                        foreach ($Tag in $Tags) 
                            {
                                ...
                                'UpdateManagement'             = $UpdateMgmt;
                                'BootDiagnostics'              = $bootdg;
                                ...
                                'DataDiskStorageType'          = [string]$StorAcc;
                                'DataDiskSizeGB'               = $dataSize;
                                'PowerState'                   = $data.extended.instanceView.powerState.displayStatus;
                                'NICName'                      = [string]$vmnic.name;
                                'NICType'                      = [string]$vmnic.properties.nicType;
                                'DNSServers'                   = [string]$vmnic.properties.dnsSettings.dnsServers;
                                'PublicIP'                     = $PIP;
                                'VirtualNetwork'               = $VNET;
                                'Subnet'                       = $Subnet;
                                'NSG'                          = $vmnsg;
                                'AcceleratedNetworking'        = [string]$vmnic.properties.enableAcceleratedNetworking;
                                'IPForwarding'                 = [string]$vmnic.properties.enableIPForwarding;
                                'PrivateIPAddress'             = $vmnic.properties.ipConfigurations.properties.privateIPAddress;
                                'PrivateIPAllocation'          = $vmnic.properties.ipConfigurations.properties.privateIPAllocationMethod;
                                'VMExtensions'                 = $ext;
                                'ResourceU'                    = $ResUCount;
