...
.Link
https://github.com/microsoft/ARI/Modules/Networking/Frontdoor.ps1
                        ...
                        foreach ($Tag in $Tags) 
                            {  
                                $obj = @{
                                    ...
                                    'Web ApplicationFirewall' = [string]$WAF;
                                    'Frontend'       = [string]$data.frontendEndpoints.name;
                                    'Backend'        = [string]$data.backendPools.name;
                                    'HealthProbe'   = [string]$data.healthProbeSettings.name;
                                    'LoadBalancing' = [string]$data.loadBalancingSettings.name;
                                    'RoutingRules'  = [string]$data.routingRules.name;
                                    'ResourceU'     = $ResUCount;
                                    'Total'          = $Total;
